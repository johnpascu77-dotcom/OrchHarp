...
| **2c — bisbigliando** ✅ | A note in the `bisbLoNote..bisbHiNote` zone (0/0 = off) is consumed and re-emitted as a **sample-clocked** measured tremolo (`bisbRate` 1/16..1/64 ±triplet; runs transport-stopped too) until its note-off; the played note IS the rustle pitch (`nearestStringNote` + octave-folded into 24..103); `bisbEnharmonic` rocks to the nearest neighbour string. `BisbVoice` list, `drainBisb`/`stopBisb`/`flushBisb`, notes on `glissEmitChannel`. A "Bisbigliando" marker logs to the sidecar. Built 2026-09-03. |
